...
class Messu extends BitBase {
	...
	function post_message( $pToLogin, $to, $cc, $bcc, $subject, $body, $priority) {
		...
		if( $userInfo ) {
			if ($gBitUser->getPreference('allowMsgs', 'y', $userInfo['user_id'] )) {
				...
				if ($this->getDb()->getOne("select count(*) from `".BIT_DB_PREFIX."messu_messages` where `to_user_id`=? and `from_user_id`=? and `hash`=?", array( $userInfo['user_id'], $gBitUser->mUserId, $hash ) ) ) {
					...
					$query = "INSERT INTO `".BIT_DB_PREFIX."messu_messages`
							  ...
							  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
					$this->getDb()->query( $query, array( $userInfo['user_id'], $gBitUser->mUserId, $to, $cc, $bcc, $subject, $body,(int) $now,'n','n','n',(int) $priority,$hash ) );
	...
	function list_messages( $pUserId, $offset, $maxRecords, $sort_mode, $find, $flag = '', $flagval = '', $prio = '' ) {
		...
		$query = "SELECT uu.`login` AS `user`, uu.`real_name`, uu.`user_id`, mm.* from `".BIT_DB_PREFIX."messu_messages` mm INNER JOIN `".BIT_DB_PREFIX."users_users` uu ON( mm.`from_user_id`=uu.`user_id` )
				  ...
				  ORDER BY ".$this->getDb()->convert_sortmode($sort_mode).",".$this->getDb()->convert_sortmode("msg_id_desc");
	...
	function flag_message( $pUserId, $msg_id, $flag, $val ) {
		...
		$this->getDb()->query($query,array($val,$pUserId,(int)$msg_id));
	}
	...
	function delete_message($pUserId, $msg_id) {
		...
		$this->getDb()->query($query,array($pUserId,(int)$msg_id));
	}
	...
	function user_unread_messages( $pUserId ) {
		return $this->getDb()->getOne( "select count( * ) from `".BIT_DB_PREFIX."messu_messages` where `to_user_id`=? and `is_read`=?",array( $pUserId,'n' ) );
	}
}
